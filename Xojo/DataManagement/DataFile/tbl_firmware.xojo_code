...
Inherits DataFile.ActiveRecordBase
	...
	#tag Method, Flags = &h0
		Shared Function FindByID(id as String) As DataFile.tbl_firmware
		  //Usage:
		  //dim tbl_firmware as DataFile.tbl_firmware = DataFile.tbl_firmware.FindByID( id )
		  dim s as string
		  s = "Select * from tbl_firmware Where uuid = '" + str(id) + "'"
		  ...
		  dim tbl_firmware as new DataFile.tbl_firmware
		  tbl_firmware.ReadRecord(rs)
		  return tbl_firmware
	...
	#tag Method, Flags = &h0
		Shared Function List(stmt as PreparedSQLStatement) As DataFile.tbl_firmware()
		  ...
		  dim aro() as DataFile.tbl_firmware
		  ...
		  while rs.eof = false
		    dim oRecord as new DataFile.tbl_firmware
	...
	#tag Method, Flags = &h0
		Shared Function List(sCriteria as string = "", sOrder as string = "", iOffset as Integer = -1) As DataFile.tbl_firmware()
		  ...
		  ars.append DataFile.tbl_firmware.BaseSQL
	...
	#tag Method, Flags = &h0
		Shared Function ListCount(sCriteria as string = "") As Integer
		  ...
		  ars.append DataFile.tbl_firmware.BaseSQL(True)
		  if sCriteria<>"" then
		    ars.append "WHERE " + sCriteria
		  end if
		  dim s as string = ars.JoinSQL
	...
	#tag Property, Flags = &h0
		sfirmware_version As String
	#tag EndProperty

	#tag Property, Flags = &h0
		sfk_inventory As String
	#tag EndProperty

	#tag Property, Flags = &h0
		supdate_comments As String
	#tag EndProperty

	#tag Property, Flags = &h0
		supdate_date As String
	#tag EndProperty
