#tag Class
Protected Class tbl_contactable_linking
Inherits DataFile.ActiveRecordBase
	...
	#tag Method, Flags = &h0
		Shared Function BaseSQL(bAsCount as Boolean = false) As String
		  ...
		  ars.Append "From tbl_contactable_linking"
	...
	#tag Method, Flags = &h0
		Shared Function FindByID(id as String) As DataFile.tbl_contactable_linking
		  //Usage:
		  //dim tbl_contactable_linking as DataFile.tbl_contactable_linking = DataFile.tbl_contactable_linking.FindByID( id )
		  dim s as string
		  s = "Select * from tbl_contactable_linking Where uuid = '" + str(id) + "'"
		  ...
		  dim tbl_contactable_linking as new DataFile.tbl_contactable_linking
		  tbl_contactable_linking.ReadRecord(rs)
		  return tbl_contactable_linking
		  
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Shared Function List(stmt as PreparedSQLStatement) As DataFile.tbl_contactable_linking()
		  ...
		  dim aro() as DataFile.tbl_contactable_linking
		  ...
		  while rs.eof = false
		    dim oRecord as new DataFile.tbl_contactable_linking
	...
	#tag Method, Flags = &h0
		Shared Function List(sCriteria as string = "", sOrder as string = "", iOffset as Integer = -1) As DataFile.tbl_contactable_linking()
		  ...
		  ars.append DataFile.tbl_contactable_linking.BaseSQL
	...
	#tag Method, Flags = &h0
		Shared Function ListCount(sCriteria as string = "") As Integer
		  ...
		  ars.append DataFile.tbl_contactable_linking.BaseSQL(True)
		  if sCriteria<>"" then
		    ars.append "WHERE " + sCriteria
		  end if
		  dim s as string = ars.JoinSQL
	...
	#tag Method, Flags = &h0
		Shared Function ListGrouped(sCriteria as string = "", sOrder as string = "", sGroupBy as String = "") As Dictionary
		  ...
		  dim oRecordList() as DataFile.tbl_contactable_linking
		  ...
		  oRecordList() = DataFile.tbl_contactable_linking.List(sCriteria, sOrder)
		  ...
		  For Each oRecord as DataFile.tbl_contactable_linking In oRecordList()
		    ...
		    For idx1 as integer = 0 To sGroupByList.Ubound
		      ...
		      If sArray.IndexOf( sGroupValue ) > -1 THen
		        ...
		        Elseif jsCurrent(n3).Value( sGroupValue ) IsA DataFile.tbl_contactable_linking Then
		        ...
		        Else
		          ...
		          Try
		            ...
		            dim oRecords() as DataFile.tbl_contactable_linking
	...
	#tag Property, Flags = &h0
		bprimary_contactable As Boolean
	#tag EndProperty
	...
	#tag Property, Flags = &h0
		sfk_parent As String
	#tag EndProperty

	#tag Property, Flags = &h0
		sparent_table As String
	#tag EndProperty


	#tag ViewBehavior
		#tag ViewProperty
			Name="bprimary_contactable"
			Group="Behavior"
			Type="Boolean"
		#tag EndViewProperty
		#tag ViewProperty
