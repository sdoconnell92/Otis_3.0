...
Inherits DataFile.ActiveRecordBase
	...
	#tag Method, Flags = &h0
		Shared Function FindByID(id as String) As DataFile.tbl_payments
		  ...
		  dim tbl_payments as new DataFile.tbl_payments
		  tbl_payments.ReadRecord(rs)
		  return tbl_payments
	...
	#tag Method, Flags = &h0
		Shared Function List(stmt as PreparedSQLStatement) As DataFile.tbl_payments()
		  ...
		  dim aro() as DataFile.tbl_payments
		  ...
		  while rs.eof = false
		    dim oRecord as new DataFile.tbl_payments
	...
	#tag Method, Flags = &h0
		Shared Function List(sCriteria as string = "", sOrder as string = "", iOffset as Integer = -1) As DataFile.tbl_payments()
		  ...
		  ars.append DataFile.tbl_payments.BaseSQL
	...
	#tag Method, Flags = &h0
		Shared Function ListCount(sCriteria as string = "") As Integer
		  ...
		  ars.append DataFile.tbl_payments.BaseSQL(True)
		  if sCriteria<>"" then
		    ars.append "WHERE " + sCriteria
		  end if
		  dim s as string = ars.JoinSQL
	...
	#tag Method, Flags = &h0
		Shared Function ListGrouped(sCriteria as string = "", sOrder as string = "", sGroupBy as String = "") As Dictionary
		  ...
		  dim oRecordList() as DataFile.tbl_payments
		  ...
		  oRecordList() = DataFile.tbl_payments.List(sCriteria, sOrder)
		  ...
		  For Each oRecord as DataFile.tbl_payments In oRecordList()
		    ...
		    For idx1 as integer = 0 To sGroupByList.Ubound
		      ...
		      If sArray.IndexOf( sGroupValue ) > -1 THen
		        ...
		        Elseif jsCurrent(n3).Value( sGroupValue ) IsA DataFile.tbl_payments Then
		        ...
		        Else
		          ...
		          Try
		            ...
		            dim oRecords() as DataFile.tbl_payments
	...
	#tag Property, Flags = &h0
		spayment_amount As String
	#tag EndProperty

	#tag Property, Flags = &h0
		spayment_date As String
	#tag EndProperty

	#tag Property, Flags = &h0
		spayment_memo As String
	#tag EndProperty

	#tag Property, Flags = &h0
		spayment_type As String
	#tag EndProperty
