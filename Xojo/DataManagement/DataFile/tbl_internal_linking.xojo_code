...
Inherits DataFile.ActiveRecordBase
	#tag Method, Flags = &h0
		Shared Function BaseSQL(bAsCount as Boolean = false) As String
		  ...
		  ars.Append "From tbl_internal_linking"
	...
	#tag Method, Flags = &h0
		Shared Function FindByID(id as String) As DataFile.tbl_internal_linking
		  //Usage:
		  //dim tbl_internal_linking as DataFile.tbl_internal_linking = DataFile.tbl_internal_linking.FindByID( id )
		  dim s as string
		  s = "Select * from tbl_internal_linking Where uuid = '" + str(id) + "'"
		  ...
		  dim tbl_internal_linking as new DataFile.tbl_internal_linking
		  tbl_internal_linking.ReadRecord(rs)
		  return tbl_internal_linking
		  
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Shared Function List(stmt as PreparedSQLStatement) As DataFile.tbl_internal_linking()
		  ...
		  dim aro() as DataFile.tbl_internal_linking
		  ...
		  while rs.eof = false
		    dim oRecord as new DataFile.tbl_internal_linking
	...
	#tag Method, Flags = &h0
		Shared Function List(sCriteria as string = "", sOrder as string = "", iOffset as Integer = -1) As DataFile.tbl_internal_linking()
		  ...
		  ars.append DataFile.tbl_internal_linking.BaseSQL
	...
	#tag Method, Flags = &h0
		Shared Function ListCount(sCriteria as string = "") As Integer
		  ...
		  ars.append DataFile.tbl_internal_linking.BaseSQL(True)
		  if sCriteria<>"" then
		    ars.append "WHERE " + sCriteria
		  end if
		  dim s as string = ars.JoinSQL
