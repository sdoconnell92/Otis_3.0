...
Protected Module StringUtils
	#tag Method, Flags = &h0
		Sub AppendArray(extends ars() as string, arsAppend() as string)
		  
		  For i as integer = 0 to arsAppend.Ubound
		    ars.Append(arsAppend(i))
		  Next
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function SQLify(Extends s As String) As String
	...
	#tag Method, Flags = &h0
		Function SQLQueryType(extends s as string) As String
		  dim selectIndex as string = Str(s.InStr( "Select" ))
		  dim insertIndex as string = Str(s.InStr( "Insert" ))
		  dim updateIndex as string = Str(s.InStr( "Update" ))
		  dim deleteIndex as string = Str(s.InStr( "Delete" ))
		  ...
		  For i1 as Integer = 0 To ari.Ubound
		    If val( Left( ari(i1), ari(i1).InStr(" :") ) ) <> 0 Then
		      i = i1
		      Exit
		    End If
		  Next
		  
		  If ari(i).InStr(" :Select") <> 0 Then
		    Return "Select"
		  ElseIf ari(i).InStr(" :Insert") <> 0 Then
		    Return "Insert"
		  ElseIf ari(i).InStr(" :Update") <> 0 Then
		    Return "Update"
		  ElseIf ari(i).InStr(" :Delete") <> 0 Then
		    Return "Delete"
		  End If
