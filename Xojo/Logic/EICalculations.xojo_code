...
Protected Module EICalculations
	#tag Method, Flags = &h1
		Protected Function CalculateGroup(aroLIStor() as RecordStorageClass, bApplyEIPLDiscount as Boolean = False) As TotalsClass
	...
	#tag Method, Flags = &h1
		Protected Function CalculateSingleLine(oLIStor as RecordStorageClass) As TotalsClass
		  ...
		  Case "LinkedFolder"
		    ...
		    For Each oLine as RecordStorageClass In oLIStor.aroChildren
		      ...
		      oRetTotals.AddTotals(oCurrentTotals)
		    Next
		  ...
		  Case "GroupFolder"
		    ...
		    aroDiscountRecords() = DataFile.tbl_group_discounts.List( "fkeipl = '" + sEIPLuuid + "' And group_name = '" + sGroupName + "'" )
		  ...
		  Case "GrandParent", "Child"
		    ...
		    If oLIStor.aroChildren <> -1 And oLIRecord.buse_children_total Then
		      // There are Children and we want to use the total generated by them
		      oRetTotals = CalculateGroup(oLIStor.aroChildren)
		      
		    ElseIf oLIStor.aroChildren = -1 Or Not oLIRecord.buse_children_total Then
