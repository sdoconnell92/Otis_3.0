...
Begin ContainerControl contPL
   ...
   Begin PagePanel ppEIPLSwitcher
      ...
      Begin PushButton pbPrintPreview
         ...
         Width           =   80
      End
      Begin contLIDetailsPL instLIDetailsPL
         AcceptFocus     =   False
         AcceptTabs      =   True
         AutoDeactivate  =   True
         BackColor       =   &cFFFFFF00
...
#tag Events instLineItemList
	...
	#tag Event
		Function evdefDoubleClick() As Boolean
		  ...
		  If instLineItemList.lbItems.ListIndex <> -1 Then
		    ...
		    If oRowTag.oTableRecord <> Nil Then
		      oRecord = oRowTag.GetTableRecordVariant
		      instLIDetailsPL.LoadMe(oRecord)
		    End If
	...
	#tag EndEvent
	#tag Event
		Function evdefListboxSettings(lb as entListbox, ByRef dictCellTypes as Dictionary, ByRef dictFieldNames as Dictionary) As String()
		  '!@! Table Dependent !@!
		  dim dictRet as New Dictionary
		  ...
		  If dictFieldNames = Nil And dictCellTypes = Nil Then
		    ...
		    // Set header names
		    s1 = "Name,Category,Description,Qty"
		    s2() = Split(s1,",")
		    lb.Heading = s2()
		    dictRet.Value("arsHeaders") = s2()
		    ...
		    sRowType = "GrandParent"
		    'field names
		    s1 = "li_name,li_category,li_description,li_quantity"
		    s2() = Split(s1,",")
		    dictFieldNames.Value(sRowType) = s2
...
#tag Events instInventoryList
	#tag Event
		Sub evdefListboxSettings(lbItems as entListbox, ByRef dictCellTypes as Dictionary, ByRef dictFieldNames as Dictionary)
		  ...
		  'cell types
		  dim iCellTypes4() as integer = Array(0,0)
		  dictCellTypes.Value(sRowType) = iCellTypes4
		  ...
		  'cell types
		  dim iCellTypes5() as integer = Array(0,0)
		  dictCellTypes.Value(sRowType) = iCellTypes5
		  ...
		  'cell types
		  dim iCellTypes6() as integer = Array(0,0)
		  dictCellTypes.Value(sRowType) = iCellTypes6
		  ...
		  'cell types
		  dim iCellTypes7() as integer = Array(0,0)
		  dictCellTypes.Value(sRowType) = iCellTypes7
